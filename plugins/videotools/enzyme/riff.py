...
class Riff(core.AVContainer):
    ...
    def _parseLIST(self, t):
        ...
        while i < size - 8:
            ...
            if key == b'LIST':
                ...
                if not key in AVIINFO.keys() and key != b'IDIT':
                ...
                if key == b'ISFT':
                    ...
                    value = value.replace(b'\0', '').lstrip().rstrip()
                value = value.replace(b'\0', '').lstrip().rstrip()
                if value:
                    retval[key] = value
    ...
    def _parseRIFFChunk(self, file):
        ...
        if name == b'LIST':
            ...
            file.seek(size, 1)
        elif not name.decode('ascii', 'replace').strip(string.printable + string.whitespace):
