...
from picard.config import BoolOption, IntOption, get_config
...
class XQAFFile(VCommentFile):
    ...
    class XQAFWithConfig(XQAF):
        ...
        def save(self, filething=None):
            ...
            gapless = None
            # Do not store the gapless tag in the metadata. Instead the
            # gapless flag in the XQAF file header will be updated.
            if self._config.setting["xqaf_update_gapless_flag"]:
                gapless = "1" in self.tags.get("gapless", [])
            if "gapless" in self.tags:
                del self.tags["gapless"]
...
class XQAFOptionsPage(OptionsPage):
    ...
    options = [
        ...
        BoolOption("setting", "xqaf_update_gapless_flag", False),
    ]
    ...
    def load(self):
        ...
        self.ui.xqaf_update_gapless_flag.setChecked(config.setting["xqaf_update_gapless_flag"])
    ...
    def save(self):
        ...
        config.setting["xqaf_update_gapless_flag"] = self.ui.xqaf_update_gapless_flag.isChecked()
